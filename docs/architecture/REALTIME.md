...
PostgreSQL owns room membership, session attempts, winner state, and the replayable safe-event sequence. Redis coordinates horizontally scaled API instances; clients never write Redis and Redis loss cannot change a winner.
...
Each player's guess is processed in the same row-locking transaction as ordinary attempts. Every duel mutation acquires the room row first and then all game rows in deterministic identifier order; simultaneous players therefore serialize without holding opposing game locks. A winning attempt records a server timestamp and establishes or joins the configured tie window. PostgreSQL row locks and constraints are authoritative, and client clocks are ignored. The final room outcome and every still-active participant game are closed in one transaction, the durable completion event is written once, and fan-out happens only after commit.
...
- **Redis fails during a room:** a committed PostgreSQL mutation remains authoritative even if publish fails; the socket disconnects and a later connection restores it from the durable cursor/snapshot. New tickets and connections fail closed until Redis recovers.
- **Publish fails after commit:** the durable event remains in PostgreSQL; clients recover through sequence replay.
