...
- PostgreSQL production target RPO: 5 minutes or better through managed point-in-time recovery.
- PostgreSQL target RTO: 60 minutes for a regional service with a rehearsed restore path.
...
Enable encrypted automated PostgreSQL backups and continuous WAL/PITR. Retain rolling backups for 35 days and protect deletion/settings changes with MFA and least privilege. Take an on-demand snapshot before any medium/high-risk migration. Store backups in a different failure domain; cross-region copies are recommended where jurisdiction and processor agreements permit.

Back up Supabase Auth according to the selected Auth/database plan. If Auth and application data are separate, both restoration points and their UTC times must be recorded. Export neither refresh tokens nor application encryption keys into the database backup location.
...
4. Run Alembic `current`; apply only migrations approved for the target application image.
5. Run integrity checks: row counts by core table, orphan foreign keys, uniqueness constraints, active/terminal state consistency, decrypt a controlled fixture for each retained key version, leaderboard eligibility sampling, and deletion tombstone reconciliation.
