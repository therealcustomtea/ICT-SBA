...
def configure_headers(document: Document) -> None:
    ...
    paragraph.text = "CLI-centred technical report  •  Baseline b3b37d3"
...
def add_front_matter(document: Document) -> None:
    ...
    for text in (
        ...
        "Technical statements are grounded in source code at commit b3b37d360396ac66893333baf6f933578d9884b0 and verification executed on 1 September 2026. This baseline includes accessible CLI terminal colours and the serialized concurrent-duel repair at commit 13fa42f0d9703f945881610a9a2d89ac7d31446f. The report does not claim that every reconstructed debugging scenario corresponds to an individually preserved historical commit; Section 7 identifies the final safeguards and their regression evidence.",
    ):
        p = document.add_paragraph()
        add_inline(p, text)
    ...
    add_inline(
        p,
        "Verification result: **120 Python tests passed** and **36 browser tests passed** with 8 intentional browser skips; focused CLI/core tests achieved **92% measured coverage**, with clean Ruff and mypy checks.",
    )
