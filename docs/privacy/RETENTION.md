...
The `mastermind-retention` command touches only application PostgreSQL rows: game sessions/attempts and dependent leaderboard links, friend challenges, rooms/members/events, and product events. Supabase Auth identities, Redis TTLs, centralized application/security logs, database backups, support-provider records, audit events, and moderation actions require the external provider or reviewed operator policies shown above; this command does not claim to enforce those lifecycles.
...
4. Globally revoke refresh sessions using the presented live Supabase access token, then delete the Auth user using a server-only admin credential. Already-issued access JWTs remain cryptographically valid until expiry, so the application tombstone denies account activity immediately.
